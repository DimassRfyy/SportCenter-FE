    ...
    <div>
      ...
      <div id="Top-Nav-Fixed" className="relative flex items-center justify-between w-full max-w-[640px] px-4 mt-[60px] z-20">
        ...
        <div className="flex items-center justify-center h-12 w-full shrink-0">
          <h1 className="font-bold text-lg leading-[27px] text-white text-center w-full">Book a Ticket</h1>
        </div>
      </div>
    </div>
  );
...
const CreateBooking = () => {
  ...
  <form action="{{ route('booking.store', $ticket) }}" method="POST"
        className="relative flex flex-col w-full px-4 gap-[18px] mt-5 pb-[30px] overflow-x-hidden">
        ...
        <div className="flex flex-col rounded-[30px] p-5 gap-[14px] bg-white">
            <div className="flex flex-col gap-[6px]">
                <label htmlFor="name" className="font-semibold text-sm leading-[21px]">Full Name</label>
                <div
            ...
            <div className="flex flex-col gap-[6px]">
                <label htmlFor="email" className="font-semibold text-sm leading-[21px]">Email Address</label>
                <div
                    className="flex items-center rounded-full px-5 gap-[10px] bg-[#F8F8F9] transition-all duration-300 focus-within:ring-1 focus-within:ring-[#F97316]">
            ...
            <div className="flex flex-col gap-[6px]">
                <label htmlFor="phone" className="font-semibold text-sm leading-[21px]">Phone</label>
                <div
                    className="flex items-center rounded-full px-5 gap-[10px] bg-[#F8F8F9] transition-all duration-300 focus-within:ring-1 focus-within:ring-[#F97316]">
            ...
            <div className="flex flex-col gap-[6px]">
                <label htmlFor="booking_date" className="font-semibold text-sm leading-[21px]">Booking Date</label>
                <div
                    className="flex items-center rounded-full px-5 gap-[10px] bg-[#F8F8F9] transition-all duration-300 focus-within:ring-1 focus-within:ring-[#F97316]">
        ...
        <div className="flex flex-col rounded-[30px] p-5 gap-6 bg-white">
            <div className="flex justify-between items-center">
                <p className="font-bold">How Many <br/>People Buying?</p>
                <div id="counter"
                    className="flex items-center justify-between w-fit min-w-[135px] rounded-full p-[14px_20px] gap-[14px] bg-[#F8F8F9]">
                    <button type="button" id="minus" className="w-6 h-6" onClick={handleMinus}>
                        <img src="assets/images/icons/minus.svg" alt="minus"/>
                    </button>
                    <p id="count-text" className="font-bold">{participants}</p>
            ...
            <div className="flex items-center justify-between">
                ...
                <p id="total-price" className="font-bold text-[22px] leading-[33px] text-[#F97316]">{formatRupiah(totalPrice)}</p>
            </div>

            <button type="submit"
                className="flex items-center justify-between p-1 pl-5 w-full gap-4 rounded-full bg-[#13181D]">
